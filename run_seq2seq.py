...
class ModelArguments:
    ...
    model_name_or_path: Optional[str] = field(
        default=None,
        metadata={            
            "help": "Path to pretrained model or model identifier from huggingface.co/models. This will be passed to `EncoderDecoderModel.from_pretrained`"
        }
    )
    encoder_model_name_or_path: Optional[str] = field(
        default=None,
        metadata={
            "help": "(Encoder) Path to pretrained model or model identifier from huggingface.co/models"
        }
    )
    ...
    tie_encoder_decoder: Optional[bool] = field(
        default=False,
        metadata={"help": "Share weights between encoder and decoder"},
    )
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        model = EncoderDecoderModel(
            encoder=AutoModel.from_pretrained(
                model_args.encoder_model_name_or_path, config=encoder_config
            ),
            decoder=AutoModelForCausalLM.from_pretrained(
                model_args.decoder_model_name_or_path, config=decoder_config
            ),
            config=encoder_decoder_config,
        )
    ...
    if model.config.decoder_start_token_id is None:
        model.config.decoder_start_token_id = decoder_tokenizer.pad_token_id
    ...
    # Data collator
    label_pad_token_id = (
        -100 if data_args.ignore_pad_token_for_loss else encoder_tokenizer.pad_token_id
    )
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                predictions = np.where(
                    predictions != -100, predictions, encoder_tokenizer.pad_token_id
                )
                predictions = encoder_tokenizer.batch_decode(
                    predictions,
